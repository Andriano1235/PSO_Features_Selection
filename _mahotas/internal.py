...
#         raise ValueError('mahotas.%s: `out` has wrong shape (got %s, while expecting %s)%s' % (
#             fname, out.shape, array.shape, detail))
#     if not out.flags.contiguous:
#         raise ValueError('mahotas.%s: `out` is not c-array%s' %
#                          (fname, detail))
#     return out
...
#         raise ValueError('mahotas.%s: `axis` is out of bounds (maximum was %s, got %s)' % (
#             fname, array.ndim, axis))
#     return axis
...
#         raise ValueError('mahotas.%s: argument is sequence, but has wrong size (%s for an array of %s dimensions)' % (
#             fname, len(value), array.ndim))
#     return value
...
#         raise TypeError(
#             'mahotas.{}: This function only accepts floating-point types (passed array of type {})'.format(function_name, A.dtype))
...
def _verify_is_integer_type(A, function_name):
    ...
    if k not in "iub":  # integer, unsigned integer, boolean
        raise TypeError('mahotas.%s: This function only accepts integer types (passed array of type %s)' % (
            function_name, A.dtype))
...
#         raise ValueError('mahotas.{0}: This function only accepts positive integer types (passed array of type {1})'.format(
#             function_name, A.dtype))
...
#         raise ValueError('mahotas.%s: this function expects an array of shape (h, w, 3), received an array of shape %s.' % (
#             funcname, arr.shape))


# def _check_2(arr, funcname):
#     if arr.ndim != 2:
#         raise ValueError('mahotas.%s: this function can only handle 2D arrays (passed array with shape %s).' % (
#             funcname, arr.shape))
